...
results = client.query("SELECT
  THREAD_ID AS STATEMENTS_THREAD_ID, EVENT_ID, EVENT_NAME AS STATEMENTS_EVENT_NAME, DIGEST, DIGEST_TEXT,
  TIMER_WAIT AS STATEMENTS_TIMER_WAIT, CURRENT_SCHEMA AS STATEMENTS_CURRENT_SCHEMA,
  OBJECT_SCHEMA AS STATEMENTS_OBJECT_SCHEMA, OBJECT_TYPE AS STATEMENTS_OBJECT_TYPE,
  OBJECT_NAME AS STATEMENTS_OBJECT_NAME, MYSQL_ERRNO, RETURNED_SQLSTATE, LOCK_TIME,
...
results = client.query("SELECT
  THREAD_ID AS WAITS_THREAD_ID, EVENT_ID, EVENT_NAME AS WAITS_EVENT_NAME, TIMER_WAIT AS WAITS_TIMER_WAIT, SPINS,
  OBJECT_SCHEMA AS WAITS_OBJECT_SCHEMA, OBJECT_NAME AS WAITS_OBJECT_NAME, INDEX_NAME,
  OBJECT_TYPE AS WAITS_OBJECT_TYPE, OPERATION AS WAITS_OPERATION, NUMBER_OF_BYTES
